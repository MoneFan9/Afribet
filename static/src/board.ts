/*
 * Plateau Songo temps réel (TypeScript — conception §6b : aucune lib JS hors HTMX).
 * Se connecte au consumer Channels (JWT en query string), affiche l'état opaque reçu
 * du serveur (serveur-autoritaire) et envoie les coups. 2×7 trous + 2 greniers.
 */
interface SongoState {
  plateau: number[];
  greniers: number[];
...
class SongoBoard {
  ...
  private state: SongoState | null = null;
  ...
  constructor(root: HTMLElement) {
    ...
    if (initialEl && initialEl.textContent) {
      try { this.state = JSON.parse(initialEl.textContent); } catch { /* ignore */ }
    }
    ...
    this.socket.onmessage = (e) => this.onMessage(JSON.parse(e.data) as StateMessage);

    const forfeit = document.getElementById("btn-forfeit");
    if (forfeit) forfeit.addEventListener("click", () => this.send({ action: "forfeit" }));
  ...
  private onMessage(msg: StateMessage): void {
    if (msg.type === "error") { this.setStatus(msg.detail || "erreur"); return; }
    if (msg.game_state) this.state = msg.game_state;
  ...
  private holesOf(idx: number): number[] {
  ...
  private render(): void {
    ...
    const row = (idx: number, clickable: boolean): string =>
      this.holesOf(idx).map((h) => {
    ...
    if (myTurn) {
      this.root.querySelectorAll<HTMLElement>("[data-move]").forEach((el) => {
...
if (boardEl) new SongoBoard(boardEl);
